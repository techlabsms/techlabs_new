...
import {CityCountryEnum, CountryEnum, getCity} from "../../enums/CountryEnum.js";
...
class ApplicationPeriod extends Component {
    ...
    state = {
        ...
        cityOptions: [{label: this.messages[CityCountryEnum.CITY[0].value], value: CityCountryEnum.CITY[0].value}]
    };
    ...
    render() {
        ...
        const countryOptions = [
            ...
            {value: CountryEnum.COLOMBIA, label: this.messages[CountryEnum.COLOMBIA]},
        ...
        return (
            <section className="py-5 my-5">
                <div className="container">
                    <Heading
                        heading={<FormattedMessage id={'app.landingpage.applicationperiod.header'}/>}
                        subheading={<FormattedMessage id={'app.landingpage.applicationperiod.subheader'}/>}
                    />
                    <div className="wave-box">
                        <div className="row">
                            <div className="d-flex col-lg-9 flex-wrap">
                                <div className="p-2 col-sm-4 mt-5">
                                    <Dropdown options={countryOptions} id={countryKey} style={dropdown_style}
                                              onSelect={this.handleCountryChange}/>
                                    <Dropdown options={this.state.cityOptions} id={cityKey} style={dropdown_style}
                                              onSelect={this.handleCityChange}/>
                                    <button className="btn btn-primary d-inline"><FormattedMessage
                                        id={'app.landingpage.applicationperiod.check'}/></button>
                                </div>
                                <div
                                    className={"p-2 col-sm-8 mt-5 " + (this.state.cityOptions[0].value !== CityCountryEnum.CITY[0].value ? 'show' : 'hidden')}>
                                    <div className="card h-100 center no-padding">
                                        <h1 className="card-title-black">
                                            {this.messages[this.state.city]} <img src={getLogo(this.state.city)}
                                                                                  className="nav-loc-icon"
                                                                                  alt="loc-ms"/>
                                        </h1> <br/>
                                        {available ? <> {open ?
                                            <p className="card-subtitle-grey"><FormattedMessage
                                                id={"app.landingpage.applicationperiod.deadline.end"}/></p> :
                                            <p className="card-subtitle-grey"><FormattedMessage
                                                id={"app.landingpage.applicationperiod.deadline.start"}/></p>
                                        }
                                            <div className="text-muted">
                                                <ApplicationCountdown date={date}/>
                                            </div>
                                            <span className="card-discover">
                                                {open ? <button className="btn btn-primary d-inline apply-button"
                                                                type="submit" onClick={this.routeToApplicationForm}>
                                                    <FormattedMessage
                                                        id={'app.landingpage.applicationperiod.deadline.apply'}/>
                                                </button> : <> </>
                                                }
                                            </span>
                                        </> : <div className="card-padding">
                                            <p className="card-subtitle-grey"><FormattedMessage
                                                id={"app.landingpage.applicationperiod.no.deadline"}/></p>
                                        </div>
                                        }
